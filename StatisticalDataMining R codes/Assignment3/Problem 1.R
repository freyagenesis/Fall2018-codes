...
LDA_train_err<-NULL
LDA_test_err<-NULL
LDA_train_err.store <- c()
LDA_test_err.store <- c()
fit <- regsubsets(crim~., data = Data_Boston_train, method = 
                    "exhaustive", nvmax = 13)
...
for (i in 1:13) {
  ...
  pred.train = predict(red.fit, newdata = red.training, type='response')$class
  pred.test = predict(red.fit, newdata = red.testing, type='response')$class
  
  pred.test <- as.numeric(as.character(pred.test))
  pred.train <- as.numeric(as.character(pred.train))
  
  test.error <- (1/length(crim_Tru_test))*sum((pred.test - crim_Tru_test)^2)
  train.error <- (1/length(crim_Tru_train))*sum((pred.train - crim_Tru_train)^2)
  
  LDA_train_err.store <- c(LDA_train_err.store, train.error)
  LDA_test_err.store <- c(LDA_test_err.store, test.error)
  
  
}

par(mfrow=c(1,2))
plot(LDA_train_err.store, type = 'l', col="Green", xlab = 
       "Number of subsets", ylab = "Training Error")
plot(LDA_test_err.store, type = 'l', col="red", xlab = 
       "Number of subsets", ylab = "Test Error")

#Min train error
min(LDA_train_err.store)
#Min test error
min(LDA_test_err.store)

##############################################
#KNN
##############################################
KNN_Data_Boston_train = Data_Boston_train
KNN_Data_Boston_test = Data_Boston_test
KNN_Data_Boston_train$crim <- as.numeric(Data_Boston_train$crim)-1
KNN_Data_Boston_test$crim <- as.numeric(Data_Boston_test$crim)-1
